...
{
    public class EulerBeam3D_v2 : IStructuralFiniteElement_v2, IEmbeddedElement_v2
    {
        ...
        private List<Element_v2> hostElementList;
        private bool[] isNodeEmbedded;
        private readonly Node_v2[][] rotNodes = new Node_v2[2][];
        ...
        public EulerBeam3D_v2(double youngModulus, double poissonRatio)
        {
        ...
        private void CalculateRotTranformation(IElement_v2 element)
        {
        ...
        private IList<Tuple<Node_v2, IList<DOFType>>> GetDOFTypesInternal(Element_v2 element)
        {
            ...
            var d = new Dictionary<Node_v2, IList<DOFType>>();
            var l = new List<Tuple<Node_v2, IList<DOFType>>>();
            foreach (var node in element.Nodes)
            {
                var embeddedNode = embeddedNodes.Where(x => x.Node == node).FirstOrDefault();
                ...
                if (embeddedNode == null)
                {
                    ...
                    d.Add(node, nodeDofs);
                    l.Add(new Tuple<Node_v2, IList<DOFType>>(node, nodeDofs));
                }
                else
                {
                    ...
                    for (int i = 0; i < hostDOFsPerNode.Count; i++)
                    {
                        ...
                        l.Add(new Tuple<Node_v2, IList<DOFType>>(embeddedNode.EmbeddedInElement.Nodes[i], hostDOFsPerNode[i]));
                    }
                }
            }
            ...
            if (uniqueDOFTypes.Length > 0)
                foreach (var node in element.Nodes)
                {
                    ...
                    l.Add(new Tuple<Node_v2, IList<DOFType>>(node, uniqueDOFTypes));
                }

            return l;
        }

        //public IList<IList<DOFType>> GetDOFTypes(Element_v2 element)
        //{
        ...
        public IList<Node_v2> GetNodesForMatrixAssembly(Element_v2 element)
        {
            var nodes = new List<Node_v2>();
            var dofTypeDictionary = GetDOFTypesInternal(element);
        ...
        private IMatrix StiffnessMatrixPure(IElement_v2 element)
        {
