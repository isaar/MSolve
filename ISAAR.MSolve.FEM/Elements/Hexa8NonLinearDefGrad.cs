...
namespace ISAAR.MSolve.FEM.Elements
{
    public class Hexa8NonLinearDefGrad : IStructuralFiniteElement, IEmbeddedHostElement
    {
        ...
        private void CalculateStrains(double[] localdisplacements, IElement element, double[][] tx_i) 
        {
            ...
            //Matrix2D[] GL = new Matrix2D[nGaussPoints]; //MS
            for (int npoint = 0; npoint < nGaussPoints; npoint++)
            {
                DGtr[npoint] = new Matrix2D(3, 3);
                //GL[npoint] = new Matrix2D(3, 3); //MS
            }
            ...
            for (int npoint = 0; npoint < nGaussPoints; npoint++)
            {
                ...
                //GLvec[npoint][5] = 2 * GL[npoint][2, 0]; //MS
            }
        ...
        private double[] UpdateForces(IElement element)
        {
            ...
            Matrix2D[] BL11a_hexa; // dimension number of gpoints
            Matrix2D[] BL12_hexa;
            Matrix2D[] BL01_hexa;
        ...
        private Matrix2D UpdateKmatrices(IElement element)
        {
            ...
            Matrix2D[] BL13_hexa;
            BL13_hexa = GetBL13Hexa(shapeFunctionNaturalDerivatives);
            Matrix2D[] BL11a_hexa; // dimension: gpoints
            Matrix2D[] BL12_hexa;
            Matrix2D[] BL01_hexa;
            BL11a_hexa = GetBL11a_hexa(J_0inv_hexa);
            BL12_hexa = GetBL12_hexa(J_0inv_hexa);
        ...
        public Tuple<double[], double[]> CalculateStresses(Element element, double[] localTotalDisplacements, double[] localdDisplacements)
        {
            ...
            return new Tuple<double[], double[]>(DefGradVec[materialsAtGaussPoints.Length - 1], materialsAtGaussPoints[materialsAtGaussPoints.Length - 1].Stresses.Data);
        ...
        public void ClearMaterialState()
        {
            ...
            //foreach (IContinuumMaterial3DDG m in materialsAtGaussPoints) m.ClearState();
        }
