...
namespace ISAAR.MSolve.IGA.Problems.SupportiveClasses
{
    public class NURBS2D
    {
		...
		public NURBS2D(int degreeKsi, int degreeHeta, double[] knotValueVectorKsi, double[] knotValueVectorHeta, NaturalPoint2D collocationPoint, IList<ControlPoint> controlPoints)
		{
			var numberOfControlPointsHeta = knotValueVectorHeta.Length - degreeHeta - 1;

			BSPLines1D bsplinesKsi = new BSPLines1D(degreeKsi, knotValueVectorKsi, new double[]{collocationPoint.Xi});
			BSPLines1D bsplinesHeta = new BSPLines1D(degreeHeta, knotValueVectorHeta, new double[] { collocationPoint.Eta });
			...
			NurbsValues = new Matrix2D(numberOfElementControlPoints, 1);
			NurbsDerivativeValuesKsi = new Matrix2D(numberOfElementControlPoints, 1);
			NurbsDerivativeValuesHeta = new Matrix2D(numberOfElementControlPoints, 1);
			NurbsSecondDerivativeValueKsi = new Matrix2D(numberOfElementControlPoints, 1);
			NurbsSecondDerivativeValueHeta = new Matrix2D(numberOfElementControlPoints, 1);
			NurbsSecondDerivativeValueKsiHeta = new Matrix2D(numberOfElementControlPoints, 1);
