...
namespace ISAAR.MSolve.XFEM.Assemblers
{
    ...
    {
        public static (SkylineMatrix Kuu, Matrix Kuc) BuildGlobalMatrix(Model2D model, IDOFEnumerator dofEnumerator)
        {
            ...
            var Kuu = SkylineBuilder.Create(numDofsUnconstrained, CalcColumnHeights(model, dofEnumerator));
            ...
            return (Kuu.BuildSkylineMatrix(), Kuc);
        }

        /// <summary>
        /// Only the entries above the diagonal count towards the column height.
        /// </summary>
        /// <param name="model"></param>
        /// <param name="dofEnumerator"></param>
        /// <returns></returns>
        private static int[] CalcColumnHeights(Model2D model, IDOFEnumerator dofEnumerator) // I vote to call them RowWidths!!!
        {
            int[] colHeights = new int[dofEnumerator.FreeDofsCount + dofEnumerator.EnrichedDofsCount];
            foreach (XContinuumElement2D element in model.Elements)
            {
