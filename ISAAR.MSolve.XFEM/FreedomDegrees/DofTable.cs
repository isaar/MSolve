...
namespace ISAAR.MSolve.XFEM.FreedomDegrees
{
    ...
    internal class DofTable: Table<Node2D, IDof, int>
    {
        public DofTable(): base()
        { }

        private DofTable(Dictionary<Node2D, Dictionary<IDof, int>> data): base(data)
        { }

        //TODO: this would be nice to have in Table too.
        public DofTable DeepCopy()
        {
            var dataCopy = new Dictionary<Node2D, Dictionary<IDof, int>>();
            foreach (var wholeRow in this.data)
            {
                // IDof and int are immutable, thus I can just copy the nested dictionary.
                dataCopy.Add(wholeRow.Key, new Dictionary<IDof, int>(wholeRow.Value));
            }
        ...
        internal void Reorder(IReadOnlyList<int> permutation, bool oldToNew)
        {
            ...
            {
                var dofIDs = new List<KeyValuePair<IDof, int>>(nodeRow);
        ...
        internal void ReorderNodeMajor(IReadOnlyList<IDof> sortedNodes)
        {
            int dofIdx = -1;
            foreach (Node2D node in sortedNodes)
            {
                bool isNodeContained = data.TryGetValue(node, out Dictionary<IDof, int> dofsOfNode);
                if (isNodeContained)
                {
                    // We cannot not update the dictionary during iterating it, thus we copy its Key collection to a list first.
                    foreach (IDof dofType in dofsOfNode.Keys.ToList()) dofsOfNode[dofType] = ++dofIdx;
                }
            }
        }
