...
namespace ISAAR.MSolve.Tests.FEM
{
    public class ContinuumElementStresses
    {
        ...
        public static void TestQuad4StrainsStresses()
        {
            ...
            (IReadOnlyList<double[]> strains, IReadOnlyList<double[]> stresses) = 
                quad4.UpdateStrainsStressesAtGaussPoints(displacements);
            Assert.True(Utilities.AreTensorsEqual(expectedStrains, strains, 1e-4));
            Assert.True(Utilities.AreTensorsEqual(expectedStresses, stresses, 1e-4));
        }
    }
}
