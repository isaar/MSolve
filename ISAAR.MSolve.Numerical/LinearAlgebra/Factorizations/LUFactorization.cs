...
{
    public class LUFactorization: ILUFactorization
    {
        private readonly int order;
        
        /// <summary>
        /// Stores the lower triangle L and upper triangle U in the same 2D array. 
        /// The unitary diagonal of L is implied.
        /// </summary>
        ...
        public LUFactorization(double[,] factorizedMatrix, int[] permutation)
        {
        ...
        /// <summary>
        /// Returns the determinant of the original matrix. det(A) = det(L*U) = det(L)*det(U). 
        ...
        /// <summary>
        /// Solves A*x = b with respect to x, using forward substitution and backward substitution. 
        /// </summary>
        ...
        public IVector Solve(IVectorView b)
        {
            ...
            double[] y = ForwardSubstitution(pb); //P*A*x=P*b => L*U*x=P*b => L*y=P*b => y=inv(L)*P*b 
        ...
        private IVector BackSubstitution(double[] y)
        {
            ...
            for (int row = order - 1; row >= 0; --row)
            {
                ...
                x[row] = nominator / data[row, row]; // denominator = diagonal entry of U != 1 (in general)
            }
