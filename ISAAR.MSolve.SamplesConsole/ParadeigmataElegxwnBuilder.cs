...
namespace ISAAR.MSolve.SamplesConsole
{
    class ParadeigmataElegxwnBuilder
    {
        ...
        public static void HexaElementsOnlyVonMises(Model model)
        {
            ...
            {
                ID = 1,
                ElementType = new Hexa8original(material1) // dixws to e. exoume sfalma enw sto beambuilding oxi//edw kaleitai me ena orisma to Hexa8
            };
            ID2 = 1;
            for (int j = 0; j < 2; j++)
            {
        ...
        public static void Hexanl_v2_CantileverBuilder(Model model, double load_value)
        {
            //xrhsimopoiithike to  ParadeigmataElegxwnBuilder.HexaCantileverBuilder(Model model, double load_value)
            // allagh tou element kai tou material
            ...
            for (int nElement = 0; nElement < elementData.GetLength(0); nElement++)
            {
                ...
                {
                    ID = nElement + 1,
                    ElementType = new Hexa8NL_v2(material1, 3, 3, 3) // dixws to e. exoume sfalma enw sto beambuilding oxi//edw kaleitai me ena orisma to Hexa8                    
                };
                for (int j = 0; j < 8; j++)
                {
                    e1.NodesDictionary.Add(elementData[nElement, j + 1], model.NodesDictionary[elementData[nElement, j + 1]]);
