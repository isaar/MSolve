...
namespace ISAAR.MSolve.Preprocessor.Meshes.GMSH
{
    ...
    internal class GmshCellFactory
    {
        ...
        private readonly IReadOnlyList<Node> allVertices;

        public GmshCellFactory(IReadOnlyList<Node> allVertices)
        {
        ...
        /// <returns></returns>
        public bool TryCreateCell(int cellCode, int[] vertexIDs, out CellConnectivity cell)
        {
            bool validCell = gmshCellCodes.TryGetValue(cellCode, out CellType type);
            if (validCell)
            {
                var cellVertices = new Node[vertexIDs.Length];
                ...
                cell = new CellConnectivity(type, cellVertices);
                return true;
            }
            else
            {
        ...
        private void FixVerticesOrder(Node[] cellVertices)
        {
            ...
            {
                Node vertex1 = cellVertices[i];
                Node vertex2 = cellVertices[(i + 1) % cellVertices.Length];
